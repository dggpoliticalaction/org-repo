...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   CREATE TYPE "public"."enum_users_social_links_link_type" AS ENUM('reference', 'custom');
  CREATE TABLE "users_social_links" (
  	"_order" integer NOT NULL,
  	"_parent_id" integer NOT NULL,
  	"id" varchar PRIMARY KEY NOT NULL,
  	"link_type" "enum_users_social_links_link_type" DEFAULT 'reference',
  ...
  ALTER TABLE "users" ADD COLUMN "author_slug" varchar;
  ...
  CREATE UNIQUE INDEX "users_author_slug_idx" ON "users" USING btree ("author_slug");
  CREATE INDEX "users_profile_image_idx" ON "users" USING btree ("profile_image_id");`)
}

export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.execute(sql`
   ALTER TABLE "users_social_links" DISABLE ROW LEVEL SECURITY;
  ...
  DROP INDEX "users_author_slug_idx";
  DROP INDEX "users_profile_image_idx";
  ALTER TABLE "users" DROP COLUMN "author_slug";
  ALTER TABLE "users" DROP COLUMN "profile_image_id";
  ALTER TABLE "users" DROP COLUMN "affiliation";
  DROP TYPE "public"."enum_users_social_links_link_type";`)
}
