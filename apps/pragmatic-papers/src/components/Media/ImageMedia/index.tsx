...
export const ImageMedia: React.FC<MediaProps> = (props) => {
  ...
  if (!src && resource && typeof resource === "object") {
    const {
      alt: altFromResource,
      blurDataURL: resourceBlurDataURL,
      height: fullHeight,
      url,
      width: fullWidth,
    } = resource
    ...
    blurDataURL = resourceBlurDataURL ?? undefined
  ...
  return (
    <NextImage
      alt={alt}
      className={cn("rounded-xs h-full w-full object-cover", imgClassName)}
      fill={fill}
      ...
      // Callers should pass an accurate `size` prop for non-full-width images so the browser fetches
      // the right srcset entry. The default assumes the image spans the full viewport width.
